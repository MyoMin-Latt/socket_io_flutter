...
class _HomeScreenState extends State<HomeScreen> {
  late IO.Socket _socket; // socket 1
  ...
  _connectSocket() {
    ...
    _socket.on(
      'message',
      (data) {
        print('_connectSocket data : $data');
        if (data['message'] == 'logout pixel') {
    ...
    _socket.on(
      'length',
      (data) {
        print('_connectSocket length : $data');
      },
    );
  }
  ...
  void initState() {
    ...
    _socket = IO.io(
      // socket 2
      Platform.isIOS ? 'http://localhost:3000' : 'http://192.168.0.109:3000',
